...
export async function GET(req: Request) {
    ...
    async (acc) => {
      try {
        ...
        const fields = [
          ...
          "adset{name,targeting,targeting_optimization_types,optimization_goal,daily_budget,lifetime_budget}",
          ...
          `insights.time_range(${timeRange}){reach,impressions,clicks,spend,cost_per_result,objective,actions}`,
        ].join(",");
        ...
        while (url) {
          ...
          const chunk = (data.data ?? []).map((ad: any) => {
            ...
            return {
              ...ad,
              budget: Number(ad.adset?.daily_budget || ad.adset?.lifetime_budget || ad.campaign?.daily_budget || ad.campaign?.lifetime_budget || 0) / 100,
              reach: insight.reach,
  ...
  const normalized = allAds
    .map((ad) => {
      ...
      const engAction = actions.find((a: any) => a.action_type === "post_engagement" || a.action_type === "page_engagement");
      const engagement = engAction ? Number(engAction.value ?? 0) : 0;
      ...
      const messages = msgAction ? Number(msgAction.value ?? 0) : 0;
