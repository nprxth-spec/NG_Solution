      ...
      <AdminCard className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <p className="text-sm text-slate-500">
            ผู้ใช้ทั้งหมด <span className="font-semibold text-slate-800">{total.toLocaleString("th-TH")}</span> ราย
          </p>
            ...
            <tbody>
              {users.map((u, idx) => (
                ...
                >
                  ...
                  <td className="py-2.5 px-3">
                    <span className={`text-sm font-semibold ${u.plan === "business" ? "text-amber-600" : "text-teal-600"}`}>
                      {u.plan === "business" ? "∞" : u.credits.toLocaleString("th-TH")}
                    </span>
                  </td>
        ...
        <AdminPagination
          ...
          buildHref={(p) => `?page=${p}${query ? `&q=${encodeURIComponent(query)}` : ""}`}
        />
      </AdminCard>
    </AdminPageShell>
  );
