...
export async function ActivityLogsContent({
  searchParams,
}: {
  ...
  return (
    <AdminPageShell
      title="Activity Logs"
      description="ดูประวัติการล็อกอิน การจัดการเซสชัน และกิจกรรมสำคัญของผู้ใช้"
    >
      <AdminCard>
        ...
        <AdminPagination
          ...
          buildHref={(p) =>
            `?page=${p}&pageSize=${pageSize}&date=${dateFilter}${from ? `&from=${from}` : ""}${to ? `&to=${to}` : ""}&category=${categoryFilter}&email=${encodeURIComponent(emailFilter)}&action=${encodeURIComponent(actionFilter)}`
          }
        />
      </AdminCard>
    </AdminPageShell>
