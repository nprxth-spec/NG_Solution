    ...
    >
      <AdminCard>
        ...
        <AdminPagination
          ...
          buildHref={(p) => `?page=${p}&pageSize=${pageSize}&date=${dateFilter}${from ? `&from=${from}` : ""}${to ? `&to=${to}` : ""}&category=${categoryFilter}&email=${encodeURIComponent(emailFilter)}&action=${encodeURIComponent(actionFilter)}`}
        />
      </AdminCard>
    </AdminPageShell>
  );
