...
export async function CentxoExportLogsContent({
  searchParams,
}: {
    ...
    >
      <AdminCard>
              ...
              {logs.map((log) => {
                ...
                const meta =
                  details && typeof details === "object" && details !== null
                    ? (details.meta as { ip?: string; sourcePath?: string } | undefined) ??
                      null
                    : null;
                const ip = meta?.ip ?? "-";
        ...
        <AdminPagination
          ...
          buildHref={(p) =>
            `?page=${p}&pageSize=${pageSize}&date=${dateFilter}${from ? `&from=${from}` : ""}${to ? `&to=${to}` : ""}&type=${typeFilter}&status=${statusFilter}`
          }
        />
      </AdminCard>
    </AdminPageShell>
