...
export function AdAccountsTab() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 min-h-0 overflow-auto w-full scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-800">
        {loading ? (
          <div className="flex items-center justify-center py-10">
            <Loader2 className="w-6 h-6 animate-spin text-gray-400" />
          </div>
        ) : error ? (
          <div className="text-sm text-red-500 bg-red-50 border border-red-100 rounded-lg px-4 py-3 m-4">
            {error}
          </div>
          ...
          <table className="w-full text-sm table-fixed border-separate border-spacing-0">
            <thead className="bg-gray-50 dark:bg-gray-800/60 text-[12px] sticky top-0 z-30 shadow-sm border-gray-200 dark:border-gray-700">
              <tr className="[&>th]:border-t [&>th]:border-b [&>th]:border-gray-200 dark:[&>th]:border-gray-700">
                  <th className="px-3 py-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-12 border-r border-gray-200 dark:border-gray-700">
                    #
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-32 border-r border-gray-200 dark:border-gray-700">
                  {isThai ? "สถานะ" : "Status"}
                </th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-64 border-r border-gray-200 dark:border-gray-700">
                  {isThai ? "บัญชีโฆษณา" : "Ad Account"}
                </th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-40 border-r border-gray-200 dark:border-gray-700">
                  {isThai ? "ยอดใช้จ่าย / ลิมิต" : "Spend / Limit"}
                </th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-40 border-r border-gray-200 dark:border-gray-700">
                ...
                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide w-28 border-r border-gray-200 dark:border-gray-700">
              ...
              <tbody>
                {filteredRows.map((row, idx) => {
                  ...
                  return (
                    <tr
                      key={row.id}
                      className="[&>td]:border-b [&>td]:border-gray-100 dark:[&>td]:border-gray-800 transition-colors bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800/60"
                    >
                      <td className="px-3 py-2 text-center text-xs text-gray-500 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700">
                        {idx + 1}
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700">
                        <div className="flex items-center h-full">
                        {row.fbStatus != null ? (
                          isActiveStatus ? (
                            <span className="inline-flex items-center gap-1.5 rounded-full bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-xs px-2 py-0.5 font-normal">
                              <ShieldCheck className="w-3.5 h-3.5" />
                              {label}
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1.5 rounded-full bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-xs px-2 py-0.5 font-normal">
                      ...
                      <td className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700">
                        <div className="flex flex-col">
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="font-normal text-gray-900 dark:text-gray-100 hover:text-primary dark:hover:text-primary cursor-pointer"
                          >
                            {row.name}
                          </a>
                      ...
                      <td className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700">
                        {hasSpendingCap(row.spendCap) && row.amountSpent != null ? (() => {
                          ...
                          if (percent >= 70 && percent < 80) barColor = "bg-yellow-400";
                          else if (percent >= 80 && percent < 90) barColor = "bg-orange-400";
                          else if (percent >= 90 && percent < 100) barColor = "bg-orange-500";
                          ...
                          return (
                            <div className="space-y-1 group">
                              <div className="flex justify-between text-[11px] text-gray-500 dark:text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity">
                                <span>{spent > 0 ? formatCurrency(spent, row.currency, isThai) : (isThai ? "ไม่มีการใช้จ่าย" : "No spend yet")}</span>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] text-gray-500 dark:text-gray-400 min-w-[3ch] text-right">{percent}%</span>
                              </div>
                            </div>
                          );
                        })() : (
                      ...
                      <td className="px-4 py-2 text-right text-sm border-r border-gray-200 dark:border-gray-700">
                        {(() => {
                          const methods = row.paymentMethods ?? [];
                          if (!methods.length) return <span className="text-xs text-gray-400 dark:text-gray-500">—</span>;
                          ...
                          return (
                            <div className="relative group inline-flex items-center justify-end gap-1.5">
                              {icon && <img src={icon} alt={primary.brand} className="w-6 h-4 object-contain" />}
                              {primary.last4 && <span className="text-xs text-gray-700 dark:text-gray-200">- {primary.last4}</span>}
                              {moreCount > 0 && <span className="text-[11px] text-gray-500 dark:text-gray-400">+{moreCount}</span>}
                              {moreCount > 0 && (
                                <div className="absolute top-full right-0 mt-1 hidden group-hover:block z-20 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg px-3 py-2 min-w-[160px]">
                                  <div className="text-[11px] font-medium text-gray-500 dark:text-gray-400 mb-1">{isThai ? "วิธีชำระเงินทั้งหมด" : "All payment methods"}</div>
                                  <div className="space-y-1">
                                    {methods.map((m, i) => (
                                      <div key={`${m.brand}-${m.last4}-${i}`} className="flex items-center gap-1.5">
                                        {getCardIconPath(m.brand) && <img src={getCardIconPath(m.brand)!} alt={m.brand} className="w-5 h-3.5 object-contain" />}
                                        <span className="text-[11px] text-gray-700 dark:text-gray-200">{m.last4 ? `- ${m.last4}` : ""}</span>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                      ...
                      <td className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700">
                        <span className="text-xs uppercase font-medium">{row.nationality ?? "-"}</span>
                      </td>
                      <td className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 border-r border-gray-200 dark:border-gray-700">
                        <span className="text-xs">{row.accountType ?? "-"}</span>
                        {row.businessName && (
                          <div className="text-[10px] text-gray-400 truncate max-w-[100px]" title={row.businessName}>
                            {row.businessName}
                          </div>
                        )}
                      </td>
                      <td className="px-4 py-2 text-right text-sm border-r border-gray-200 dark:border-gray-700">
                        <span className="text-xs text-gray-700 dark:text-gray-200">{row.currency ?? "-"}</span>
                      </td>
                      <td className="px-4 py-2 text-right text-sm border-r border-gray-200 dark:border-gray-700">
                        <span className="text-xs text-gray-700 dark:text-gray-200 whitespace-nowrap">{formatTimezoneDisplay(row.timezone)}</span>
                      </td>
                    </tr>
                  );
                })}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{isThai ? "ตั้งค่า Spending Limit" : "Set Spending Limit"}</DialogTitle>
            <DialogDescription>
              {editingRow ? (hasSpendingCap(editingRow.spendCap) ? (isThai ? "ควบคุมค่าใช้จ่ายโฆษณารวมของบัญชีนี้ด้วย Spending Limit" : "Control total ad spend for this account with a spending limit.") : (isThai ? "ตั้งค่า Spending Limit ครั้งแรกสำหรับบัญชีนี้" : "Create a new spending limit for this account.")) : ""}
            </DialogDescription>
          </DialogHeader>
          {editingRow && (
            <div className="space-y-4 mt-2">
              <div className="text-sm rounded-lg bg-gray-50 dark:bg-gray-900/40 p-3 border border-gray-100 dark:border-gray-800">
                <div className="font-medium text-gray-900 dark:text-gray-100">{editingRow.name}</div>
                <div className="text-xs text-gray-500 dark:text-gray-400">{editingRow.accountId}</div>
                <div className="mt-2 text-xs text-gray-600 dark:text-gray-300">
                  {isThai ? "ยอดใช้ไป" : "Spent"}: {formatCurrency(editingRow.amountSpent ? Number(editingRow.amountSpent) / 100 : 0, editingRow.currency, isThai)} · {isThai ? "Limit ปัจจุบัน" : "Current limit"}: {formatSpendingCap(editingRow.spendCap ?? null, editingRow.currency, isThai)}
                </div>
              </div>
              <div className="space-y-2">
                <div className="text-xs font-medium text-gray-700 dark:text-gray-200">{isThai ? "เลือกการทำงาน" : "Choose an action"}</div>
                {hasSpendingCap(editingRow.spendCap) ? (
                  ...
                  <div className="text-xs text-gray-500 dark:text-gray-400">{isThai ? "บัญชีนี้ยังไม่มี Spending Limit เลือกจำนวน Limit ใหม่ด้านล่าง" : "This account has no spending limit yet. Set a new limit below."}</div>
                )}
              </div>
              {mode === "change" && (
                <div className="space-y-1">
                  <label className="text-xs font-medium text-gray-700 dark:text-gray-200">{isThai ? "Limit ใหม่" : "New spending limit"}</label>
                  <div className="flex items-center gap-2">
                    <Input type="number" min={0} step="0.01" value={newLimit} onChange={(e) => setNewLimit(e.target.value)} className="h-9 text-sm" />
                    <span className="text-xs text-gray-500 dark:text-gray-400">{editingRow.currency || "USD"}</span>
                  </div>
                </div>
              )}
            </div>
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (mode === "change") {
                    ...
                    if (!Number.isFinite(v) || v <= 0) { setSaving(false); return; }
                    payload.newLimit = v;
                  }
                  const res = await fetch("/api/facebook/ad-accounts/spend-cap", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
                  const data = await res.json();
                  if (res.ok && !data.error) { setDialogOpen(false); setReloadKey((k) => k + 1); }
                } finally { setSaving(false); }
              }}
            >
              {saving ? (isThai ? "กำลังบันทึก..." : "Saving...") : (mode === "change" ? (isThai ? "เปลี่ยน Limit" : "Change limit") : mode === "reset" ? (isThai ? "รีเซ็ต Limit" : "Reset limit") : (isThai ? "ลบ Limit" : "Delete limit"))}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
